...
static struct predictor i_pred;
static struct predictor j_pred;
...
partition(unsigned int a[], int l, int r)
{
	...
	for(;;)
	{
		...
		branch_not_taken(&i_pred);

		while (less(v, a[--j]))
		{
			branch_taken(&j_pred);
		}
		branch_not_taken(&j_pred);
...
//	init_predictor(&i_pred);
//	init_predictor(&j_pred);
	...
	printf("i_pred:\n");
	dump_predictor_info(stdout, &i_pred);
	printf("j_pred:\n");
	dump_predictor_info(stdout, &j_pred);
