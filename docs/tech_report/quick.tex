...
check is done to fetch a sentinel from the appropriate sub-file if required.
This check isn't very expensive, and is significantly cheaper than either a
bounds check or putting a sentinel into every sub-file.
...
the indiviual branches in base quicksort. The \cc{i} branch is the loop control
branch for the index sweeping from left-to-right up the array during partition.
The \cc{j} branch is the corresponding loop sweeping down the array. The
partition step terminates when $i \geq j$, and the \cc{partition end} branch
checks that condition. The \cc{insertion} branch is the comparison branch in the
...
\cc{median} branch is the sum of the results for the branches involved in
computing the median-of-three for each partition.

The only difference between cache quicksort and base quicksort is that
...
iterations of the \cc{i} and \cc{j} loops, with a corresponding reduction in the
number of mispredictions. Interestingly, the percentage misprediction rate of
the \cc{i} and \cc{j} branches does not change --- it is 37\% in both cases.
...
quicksorts-3} shows additional measures of the \cc{binary left} and \cc{binary
right} branches, which are the branches within the binary search code. The
number of binary search branches is much greater than the reduction in \cc{i}
and \cc{j} branches. Furthermore, these binary search branches are highly (43\%)
unpredictable.
...
When we originally looked at quicksort, we expected that the \cc{i} and \cc{j}
...
of the section of the array to be partitioned. Thus, the \cc{i} and \cc{j}
branches are almost always biased, which makes them predictable. According to
Knuth, if the pivot element is chosen randomly, then the \cc{i} and \cc{j}
branches will be taken (i.e.  the \cc{i} or \cc{j} loop will execute) twice as
...
see that the \cc{i} and \cc{j} branches are indeed taken about twice as often as
not-taken, and the \cc{i} and \cc{j} branches are correctly predicted around
71\% of the time.
...
pronounced because the \cc{i} and \cc{j} branches will be biased in opposite
directions.
...
right-hand end of the region to be partitioned. Thus, the \cc{i} loop will
execute a large number of times, and thus the \cc{i} loop will be biased towards
being taken. The \cc{j} loop, on the other hand, will be biased toward
not-taken, because the average element is likely to be smaller than the pivot.
Therefore, the \cc{j} loop will execute only a small number of times. Thus, we
see that in a particular partition, we do not normally get an equal number of
executions of the \cc{i} and \cc{j} loops. Instead we get a large number of
...
overall the \cc{i} and \cc{j} branches are taken twice as often as not-taken, on
a particular partition one will usually be biased in the not-taken direction,
and as a result will be executed a relativelty small number of times.

In the preceding paragraphs we have seen that the \cc{i} and \cc{j} branches are
...
executions of the \cc{i} and \cc{j} branches falls by around 15\%. This fall is
...
remaining \cc{i} and \cc{j} branches. The no-median \cc{i} and \cc{j} branches
are 66\% biased, but the median-of-three branches are only 60\% biased in the
taken direction. The accuracy with which these branches are correctly predicted
falls from 71\% to 64\%.
