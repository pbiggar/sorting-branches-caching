...
%TODO why is this? the only reason I've come up with is that in the direct
%ampped cache, the same area is thrashed, while keeping keys that we;re going to
%reuse is a separte bit. in the fully assoc cache, those keys get washed out by
%the keys that arent going to be reused.
...
%TODO is a pseudo-median explained
...
We have observed before (see Section \ref{heapsort entropy}), that the more
branches an algorithm has, the more of them will be correctly predicted. we
observe that using a better pivot reduces the number of exchanges made by
quicksort. We expect to find, however, that using a better pivot reduces the
number of correct predictions, and not the number of incorrect predictions.

Each of the \code{while} loops in \code{partition} is a branch, and each
iteration of that loop corresponds to that branch being taken, while each
exchange corresponds to that branch being not-taken. So the fewer 
...
To test this theory, we create variations of quicksort using different medians.
We tested no median (or median-of-one), median-of-three, and pseudo-medians-of
-five, -seven and -nine. A pseudo-median-of-five is achieved by choosing five
keys, evenly spaces across the array, including the first and last key. The
median of the middle three keys is determined, and the pivot is chosen as the
median of the chosen key, the first key and the last key. The median-of-seven is chosen
similarly, using seven evenly spaced keys, and finding the median of the first
three and the median of the last three, and choosing as the pivot the median of
these keys and the middle key. The median of nine determines the median of three
median-of-threes, involving the first, middle and last three keys from nine
evenly spaced keys.

We measured the number of predictions at the same points as before, aswell as
when the predictors were being calculated, expecting that a reduction in misses
in the partitioning due to using a better median would be made up for by an
increase in number of mispredictions while finding the median. The results are
shown in Figure \ref{Branch prediction performance for base quicksort without a
median, with median of 3, and with pseudo medians of 5, 7 and 9}.

Picking a better pivot reduces the number of brnahces, but doesnt reduce th
enumber of mispredictions, in fact, it increases them.
