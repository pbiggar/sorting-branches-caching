...
Figure \ref{mergesorts-3}, indicating a possible minimum number. `\n{Left}' and
`\n{Right}' refer to iterating through the first (or left-hand) and second (or
...
`\n{Left}' and `\n{Right}' in the labels refer to the same as in
\ref{mergesorts-3}, while `\n{Forwards}' and `\n{Reverse}' refer to whether the
first or second part of the bitonic array segment is being merged.

This predilection is likely to be due to the fact that the final merge goes
...
In both double-aligned tiled mergesort and base mergesort, approximately two
...
heapsort curve}). Note that `\n{comparison 0}' occurs every time, and would be
