...
The relationship between this number and $H_N-1$ is relatively straightforward.
...
branch will not be taken, and the predictor will always predict not-taken. The
graph shows us the rate of correct predictions, and each point is calculated
using a seperate branch predictor. As a result, we expect $1-y = \frac{1}{x+1}$,
where $x$ and $y$ are the cartesian coordinates of each point on the graph.
$1-y$ is the number of mispredictions from the graph, while $\frac{1}{x+1}$ is
the increase of $H_N$ for step $N$.

When the array is short, as in this example, the branch predictor has too much
feedback to be as accurate as $H_N$. For $N=2$, we would approximate a 33\%
...
we approximate 12.5\% and measure 12.7\%. 
