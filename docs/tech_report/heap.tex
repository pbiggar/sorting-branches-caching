...
branch in the base heapsort algorithm. The `\cc{Creation, has children}' branch
refers to the check as to whether a key has children, during the creation of the
heap The `\cc{Creation, child comparison}' branch is the comparison between the
two children in the 2-heap, and the `\cc{Creation, child promotion}' refers to
checking if the child is larger than it's parent. The same checks are later made
during the destruction of the heap, and are referred to as `\cc{Destruction, has
children}', `\cc{Destruction, child comparison}' and `\cc{Desruction child
promotion}'.

In the memory-tuned heapsort, the creation phase is simpler, involving merely
one branch, called `\cc{fix-up}' here. The other columns refer to the
