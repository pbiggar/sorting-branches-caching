...
base heapsort experiences a sharp increase in cache misses. Cache heapsort also
has a similar increase, though LaMarca's results show that these occur at the
same position in the graph, in other words, when the set size is the same. Our
results, however, show that this occurs earlier in memory-tuned heapsort, due to the
