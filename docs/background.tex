...
in-place sort does not, though it may use a large stack, especially in recursive
algorithms.

A \n{stable} sort is one in which records which have the same key stay in the
same order across the sort. All the elementary sorts are stable, as is
mergesort, heapsort and radixsort. Quicksort is not stable.
...
The ADT interface in this project is taken from \cite{Sedgewick02}. An \cc{Item}
...
used for ADTs is in Figure \vref{Definitions used by the ADT}. Items in this
project were \cc{unsigned integer}s, and were compared using a simple
\n{less-than} function\footnote{This abstraction broke down in several cases,
such as where a \n{greater-than} function was required, such as in quicksort}.
In this case, a record and key are equivalent, and the word \n{key} is used to
refer to items to be sorted.
...
discussed in Chapter \ref{Radixsort}.
...
disk controllers.}. Several optimisations in later chapters use the address to
...
problem is not encountered in simulations in this project, as SimpleScalar
...
algorithms in this project are designed to take advantage of locality. Spatial
...
For this project, I split branches into two types. The first is a \n{flow
...
\n{dynamic}. This project is only interested in dynamic predictors, due to
their high levels of accuracy. The other types are none-the-less discussed
...
proportion to their Order, but different constants of proportionality are used
for different algorithms. An algorithm with $10N$ instructions is $O(N)$, and is
not differentiated from an algorithm with $1000N$ instructions by this
notation.
