...
The only other optimisation used is to unroll the loop while destroying the
...
%TODO this isnt true is it? placing the nsentinel is cheaper than a bounds check
%(a write to a variable thats going to be written to in a second vs a comparison?)
%and there arent an extra logN instructions, theres logN times as many

\section{Results}
All simulations in this project are performed on 32 byte cache lines. As a
result, 8-heaps are used for heapsort, since 8 4-byte \n{int}s fit the cache line.
...
\plot{heap}{}{}{Cache heapsort (4-heap) was also tested with a 16 byte cache line}{Cache heapsort (4-heap) was also tested with a 16 byte cache line}{}{}
...
\sixplot{base_heapsort-0.eps}{cache4_heapsort-0.eps}{cache_heapsort-0.eps}{cache_heapsort-1.eps}{}{}{base and cache heapsorts}
...
Figure \ref{base and cache heapsorts-1} show
that the time taken to create the heap is not significant relative to the time
taken to sort from the heap.
...
%TODO make a pretty graph for this.
