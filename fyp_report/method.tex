...
Several steps were necessary to prepare for the project. The first of these was
...
known ahead of time. One configuration of the \cc{simple} script (see
Section \ref{Simple Script}) ran quick tests using the \cc{sim-fast} simulator,
...
\cc{SimpleScalar} is an architecture simulator. It works by compiling code into its
native instruction set, PISA, then emulating a processor as it runs the program.
SimpleScalar can be found at \n{http://www.simplescalar.com}. Documentation on how
...
Several simulators are then provided: \cc{sim-fast} only counts instruction,
quickly; \cc{sim-cache} simulates a cache and counts level 1 and level 2 cache
...
The output of SimpleScalar is human readable (see Section \ref{Sample
...
file for later retrieval. The source code to the script, \cc{simple}, is in
section \ref{Simple Script}.  A sample list of commands executed by \cc{simple} are
in section \ref{Simple List}. The data file produced was suitable to reading
with a \cc{diff} program.
...
The algorithms in this project were written in C using \cc{vim}, a powerful text
editor. \cc{gcc} was used to compile the framework, when not using SimpleScalar.
The report was written using \LaTeX, and diagrams were written is
\cc{postscript}, or drawn using \cc{OpenOffice.org}, an open-source office
suite. Many files had to be edited in the same way, and this was accomplished
either using \cc{vim} macros, or by writing a short \cc{perl} script.

\section{Implementation of sorts}

The \cc{C} code written for this project was approximately 8000 lines long.
...
Each of the three $O(NlogN)$ sorts was treated in this fashion. The mergesorts
...
compiling SimpleScalar with the \cc{-03} flag, which performs a significant
...
\cc{lib-cheetah} is a SimpleScalar library used to simulate multiple
